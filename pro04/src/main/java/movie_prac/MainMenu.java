...
class MainMenu extends AbstractMenu{
	...
	private void checkReservation() {
		System.out.print("예매 발급번호를 입력하세요: ");
		try {
			Reservation r=
					reservationDao.findByResId(Integer.parseInt(scanner.nextLine()));
			if (r != null) {
				System.out.println(">> [확인 완료]" + ", 예매발급번호: " + r.getResid() + 
						", 좌석 " + r.getSeat() + ", 영화이름: " + r.getMoviename());
			}else {
	...
	private void reserve() {
		try {
			...
			System.out.println("예매할 영화발급번호를 입력하세요: ");
